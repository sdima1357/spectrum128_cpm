...
static int bI = 0;
					...
					for(yt=0;yt<8;yt++)
					{
						...
						if(SINCLAIR_FLAGS.bColor)
						{
							for(bit=0;bit<8;bit++)
							{
								...
								//int Gc = (bt?(GInc):(Gpap));
								//int Bc = (bt?(BInc):(Bpap));
								//buffB[bI][yt][7-bit] = pp[bt];//color_convertRGB_to161(Rc,Gc,Bc);
								buffB[bI][yt][bit] = __REV16(pp[bt]);//color_convertRGB_to161(Rc,Gc,Bc);
...
#if 1
								int bt          =((val>>(7-bit))&1);
								uint16_t color = __REV16(pp[bt]);
								uint8_t btt[3];
								color_convert(color,btt);
								int FC = ((int)btt[0]+btt[1]+btt[2])/3;
								btt[0] = FC;btt[1] = FC;btt[2] = FC;
								buffB[bI][yt][bit] = color_convertRGB_to16(btt);
							}
						}
#endif
						//~ LCD_Write8line(xScr,yScr,buff) ;
	  ...
	  //if((SINCLAIR_FLAGS.frameCounter&3)==0)
		...
		for(int line = 0;line< SPECTRUM_HEIGHT+BORDER_HEIGHT*2;line++)
			{
			if((
