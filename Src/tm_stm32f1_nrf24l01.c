...
uint8_t TM_SPI_Send(SPI_HandleTypeDef hspi1,uint8_t data)
{
	uint8_t  dummy = 0;
	HAL_SPI_TransmitReceive(&hspi1,&data,&dummy,1,0xffff);
	return dummy;
}
void TM_SPI_SendMulti(SPI_HandleTypeDef hspi1, uint8_t *dataO, uint8_t * dataI, int count)
{
	HAL_SPI_TransmitReceive(&hspi1,dataO,dataI,count,0xffff);
}
void TM_SPI_ReadMulti(SPI_HandleTypeDef hspi1,uint8_t* data,uint8_t dummy,int count)
{
	int k;
	for(k=0;k<count;k++)
	{
	...
	HAL_SPI_TransmitReceive(&hspi1,data,data,count,0xffff);
}

void TM_SPI_WriteMulti(SPI_HandleTypeDef hspi1,uint8_t* data, int count)
{
	HAL_SPI_Transmit(&hspi1,data,count,0xffff);
}
...
uint8_t nRF24_Check(void) {
	...
	TM_NRF24L01_WriteRegisterMulti(NRF24L01_REG_TX_ADDR, ptr, 5);
	TM_NRF24L01_ReadRegisterMulti(NRF24L01_REG_TX_ADDR, rxbuf, 5);
...
uint8_t TM_NRF24L01_Init(uint8_t channel, uint8_t payload_size) {
	...
	TM_NRF24L01_WriteRegister(NRF24L01_REG_EN_AA, 0x0);
...
void TM_NRF24L01_SetMyAddress4(uint8_t *adr) {
	NRF24L01_CE_LOW;
	TM_NRF24L01_WriteRegisterMulti(NRF24L01_REG_RX_ADDR_P1, adr, 4);
	NRF24L01_CE_HIGH;
}
void TM_NRF24L01_SetMyAddress3(uint8_t *adr) {
	NRF24L01_CE_LOW;
	TM_NRF24L01_WriteRegisterMulti(NRF24L01_REG_RX_ADDR_P1, adr, 3);
	NRF24L01_CE_HIGH;
}
void TM_NRF24L01_SetMyAddress2(uint8_t *adr) {
	NRF24L01_CE_LOW;
	TM_NRF24L01_WriteRegisterMulti(NRF24L01_REG_RX_ADDR_P1, adr, 2);
	NRF24L01_CE_HIGH;
}
void TM_NRF24L01_SetMyAddress1(uint8_t *adr) {
	NRF24L01_CE_LOW;
	TM_NRF24L01_WriteRegisterMulti(NRF24L01_REG_RX_ADDR_P1, adr, 1);
	NRF24L01_CE_HIGH;
}
void TM_NRF24L01_SetMyAddress0(uint8_t *adr) {
	NRF24L01_CE_LOW;
	TM_NRF24L01_WriteRegisterMulti(NRF24L01_REG_RX_ADDR_P1, adr, 0);
	NRF24L01_CE_HIGH;
}
...
uint8_t TM_NRF24L01_ReadABit(uint8_t reg) {
	uint8_t tmp;
	tmp = TM_NRF24L01_ReadRegister(reg);
	return tmp;
}
...
void TM_NRF24L01_SoftwareReset(void) {
	...
	TM_NRF24L01_WriteRegister(NRF24L01_REG_SETUP_AW, 	2);
